...
# Fungsi untuk mendapatkan info klasifikasi
def get_classification_info(predictions):
    ...
    return class_name, max_percentage, max_percentage < 70
...
async def klasifikasi(file: UploadFile = File(...)):
    ...
    # Get predictions
    predict, _ = data.getPrediction(img)  # Mengabaikan variabel index
    
    # Get classification info
    class_name, max_percentage, less_accurate = get_classification_info(predict)

    response = {
        "Warna Kulit": class_name,
        "Persentase": f"{max_percentage:.2f}%"
    }

    if less_accurate:
        response["Keterangan"] = "Proses deteksi kurang akurat"
...
async def klasifikasi(file: UploadFile = File(...)):
    ...
    if contours:
        ...
        return {"Warna Kulit": class_name, "Persentase": f"{max_percentage:.2f}%"}
